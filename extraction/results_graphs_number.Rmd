...
```{r}
extract$referencia<-gsub("Arguemntation","Argumentation",extract$referencia)
```


```{r}
lP<-readLines("../../extraction/listPaper.txt")
modifTitle<-tolower(gsub("[[:space:]]+"," ",gsub("[^[:alpha:]]","",gsub("\\n"," ", extract$referencia))))
modifRef<-tolower(gsub("[[:space:]]+"," ",gsub("[^[:alpha:]]","",gsub("\\n"," ",lP))))
A<-lapply(lapply(modifTitle,grep,modifRef),function(x)ifelse(length(x)==1,x,NA))
res<-data.frame(title=1:length(A), refComp=unlist(A))
res[is.na(res$refComp),]
extract$citation<-lP[res$refComp]
```
...
### Comparison with Luisa's themes
...
### Comparison with local examples in the interventions
